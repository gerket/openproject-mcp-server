...
from src.tools.work_packages import (
    list_work_packages, search_work_packages, create_work_package, delete_work_package,
    list_types, list_statuses, list_priorities, CreateWorkPackageInput
)
...
async def test_list_work_packages_empty():
    with patch('src.tools.work_packages.get_client') as mock_get_client:
        ...
        mock_client.get_work_packages = AsyncMock(return_value={"_embedded": {"elements": []}, "total": 0})
        mock_get_client.return_value = mock_client
        result = await list_work_packages()
        assert "No work packages found" in result, f"Expected empty message, got: {result}"
...
async def test_list_work_packages_results():
    with patch('src.tools.work_packages.get_client') as mock_get_client:
        mock_client = AsyncMock()
        mock_client.get_work_packages = AsyncMock(return_value={"_embedded": {"elements": [
            {"id": 10, "subject": "Fix bug", "_embedded": {
                "type": {"name": "Bug"}, "status": {"name": "New"}, "priority": {"name": "Normal"}
            }},
            {"id": 11, "subject": "Add feature", "_embedded": {
                "type": {"name": "Feature"}, "status": {"name": "In Progress"}, "priority": {"name": "High"}
            }}
        ]}, "total": 2})
        mock_get_client.return_value = mock_client
        result = await list_work_packages()
        assert "Fix bug" in result, f"Expected 'Fix bug' in result, got: {result}"
        assert "Add feature" in result, f"Expected 'Add feature' in result, got: {result}"
...
async def test_search_work_packages():
    with patch('src.tools.work_packages.get_client') as mock_get_client:
        mock_client = AsyncMock()
        mock_client.get_work_packages = AsyncMock(return_value={"_embedded": {"elements": [
            {"id": 22, "subject": "login issue", "_embedded": {
                "type": {"name": "Bug"}, "status": {"name": "New"}, "priority": {"name": "High"}
            }}
        ]}, "total": 1})
        mock_get_client.return_value = mock_client
...
async def test_create_work_package():
    with patch('src.tools.work_packages.get_client') as mock_get_client:
        mock_client = AsyncMock()
        mock_client.create_work_package = AsyncMock(return_value={
            "id": 55, "subject": "New task",
            "_embedded": {
                "type": {"name": "Task"}, "status": {"name": "New"}, "priority": {"name": "Normal"}
            }
        })
        mock_get_client.return_value = mock_client
        result = await create_work_package(CreateWorkPackageInput(project_id=4, subject="New task", type_id=1))
...
async def test_list_types():
    with patch('src.tools.work_packages.get_client') as mock_get_client:
        mock_client = AsyncMock()
        mock_client.get_types = AsyncMock(return_value={"_embedded": {"elements": [
            {"id": 1, "name": "Bug"},
            {"id": 2, "name": "Feature"}
        ]}})
        mock_get_client.return_value = mock_client
...
async def test_list_statuses():
    with patch('src.tools.work_packages.get_client') as mock_get_client:
        mock_client = AsyncMock()
        mock_client.get_statuses = AsyncMock(return_value={"_embedded": {"elements": [
        ...
        assert "In Progress" in result, f"Expected 'In Progress' in result, got: {result}"
...
async def test_list_priorities():
    with patch('src.tools.work_packages.get_client') as mock_get_client:
        mock_client = AsyncMock()
        mock_client.get_priorities = AsyncMock(return_value={"_embedded": {"elements": [
