...
from src.tools.connection import test_connection, check_permissions
...
async def test_test_connection():
    with patch('src.tools.connection.get_client') as mock_get_client:
        ...
        assert "API connection successful!" in result, f"Expected success message, got: {result}"
        assert "**Instance Version**:" in result, f"Expected Instance Version in result, got: {result}"
...
async def test_check_permissions():
    with patch('src.tools.connection.get_client') as mock_get_client:
        mock_client = AsyncMock()
        mock_client.check_permissions = AsyncMock(return_value={
            "name": "Tom Gerke",
        ...
        assert "✅" in result, f"Expected ✅ in result, got: {result}"
        assert "User Permissions Retrieved" in result, f"Expected permissions header, got: {result}"
