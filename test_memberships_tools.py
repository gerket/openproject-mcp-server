...
from src.tools.memberships import list_memberships, create_membership, delete_membership, CreateMembershipInput
...
async def test_list_memberships_empty():
    with patch('src.tools.memberships.get_client') as mock_get_client:
        ...
        mock_get_client.return_value = mock_client
        result = await list_memberships(project_id=4)
        assert "No memberships found" in result, f"Expected empty message, got: {result}"
...
async def test_create_membership():
    with patch('src.tools.memberships.get_client') as mock_get_client:
        mock_client = AsyncMock()
        mock_client.create_membership = AsyncMock(return_value={
            "id": 42,
            "_embedded": {
        ...
        mock_get_client.return_value = mock_client
        result = await create_membership(CreateMembershipInput(project_id=4, user_id=5, role_id=1))
...
async def run_all():
    tests = [
        ...
        test_list_memberships_empty,
        test_create_membership,
        test_delete_membership,
    ]
    for t in tests:
        await t()
