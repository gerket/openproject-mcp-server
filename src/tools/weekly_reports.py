...
class GenerateWeeklyReportInput(BaseModel):
    ...
    sprint_goal: str | None = Field(None, description="Optional sprint goal text")
    team_name: str | None = Field(None, description="Optional team/squad name")
...
@mcp.tool(tags={"read", "reports", "situational", "situational-read", "all"})
...
@mcp.tool(tags={"read", "reports", "all"})
...
async def generate_last_week_report(
    ...
    try:
        ...
        return format_error(f"Failed to generate last week's report: {e!s}")
