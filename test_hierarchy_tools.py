...
from src.tools.hierarchy import list_work_package_children, set_work_package_parent, remove_work_package_parent
...
async def test_list_work_package_children_empty():
    with patch('src.tools.hierarchy.get_client') as mock_get_client:
        ...
        mock_client.get_work_package_children = AsyncMock(return_value={
...
async def test_list_work_package_children_results():
    with patch('src.tools.hierarchy.get_client') as mock_get_client:
        ...
        mock_client.get_work_package_children = AsyncMock(return_value={
            "_embedded": {"elements": [
                {"id": 11, "subject": "Child task 1", "_embedded": {
        ...
        mock_get_client.return_value = mock_client
        result = await list_work_package_children(10)
        assert "Child task 1" in result, f"Expected child subject in result, got: {result}"
...
async def test_set_work_package_parent():
    with patch('src.tools.hierarchy.get_client') as mock_get_client:
        mock_client = AsyncMock()
        mock_client.update_work_package = AsyncMock(return_value={
            "id": 20, "subject": "Child WP", "_embedded": {}
        })
        mock_get_client.return_value = mock_client
