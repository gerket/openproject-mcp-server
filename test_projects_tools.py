...
from src.tools.projects import list_projects, get_project, create_project, delete_project, CreateProjectInput
...
async def test_list_projects_empty():
    with patch('src.tools.projects.get_client') as mock_get_client:
        ...
        mock_client.get_projects = AsyncMock(return_value={"_embedded": {"elements": []}})
...
async def test_get_project():
    with patch('src.tools.projects.get_client') as mock_get_client:
        ...
        mock_client.get_project = AsyncMock(return_value={
            "id": 4, "name": "infrastructure", "identifier": "infra",
...
async def test_create_project():
    with patch('src.tools.projects.get_client') as mock_get_client:
        ...
        mock_client.create_project = AsyncMock(return_value={
            "id": 99, "name": "Test Project", "identifier": "test-proj",
            "public": False, "status": "active", "active": True
        })
        mock_get_client.return_value = mock_client
        result = await create_project(CreateProjectInput(name="Test Project", identifier="test-proj"))
