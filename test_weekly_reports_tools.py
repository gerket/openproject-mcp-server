...
from src.tools.weekly_reports import generate_this_week_report, generate_last_week_report

# Unwrap FunctionTool wrappers so tools are directly callable
generate_this_week_report = generate_this_week_report.fn
generate_last_week_report = generate_last_week_report.fn
...
def make_mock_client():
    ...
    mock_client.get_memberships = AsyncMock(return_value={
        "_embedded": {"elements": []}
    })
    mock_client.get_time_entries = AsyncMock(return_value={
        "_embedded": {"elements": []}
    })
