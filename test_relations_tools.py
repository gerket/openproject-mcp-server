...
from src.tools.relations import (
    list_work_package_relations, create_work_package_relation,
    delete_work_package_relation, CreateRelationInput
)
...
async def test_list_relations_empty():
    with patch('src.tools.relations.get_client') as mock_get_client:
        ...
        mock_client.list_work_package_relations = AsyncMock(return_value={
            "_embedded": {"elements": []}
        })
...
async def test_create_relation():
    with patch('src.tools.relations.get_client') as mock_get_client:
        mock_client = AsyncMock()
        mock_client.create_work_package_relation = AsyncMock(return_value={
            "id": 77, "type": "relates",
            "_embedded": {
                "from": {"subject": "WP A", "id": 10},
                "to": {"subject": "WP B", "id": 11}
        ...
        mock_get_client.return_value = mock_client
        result = await create_work_package_relation(CreateRelationInput(from_id=10, to_id=11, type="relates"))
        assert "77" in result, f"Expected relation ID in result, got: {result}"
        assert "relates" in result, f"Expected relation type in result, got: {result}"
        assert "**ID**:" in result, f"Expected ID field label in result, got: {result}"
        assert "**Type**:" in result, f"Expected Type field label in result, got: {result}"
